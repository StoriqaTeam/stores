    ...
    > Controller for ControllerImpl<T, M, F>
{
    ...
    fn call(&self, req: Request) -> ControllerFuture {
        ...
        let fut = match (&req.method().clone(), self.static_context.route_parser.test(req.path())) {
            ...
            // GET /stores/:id/validate_update
            (&Get, Some(Route::StoreValidateUpdate(store_id))) => serialize_future(service.validate_update_store(store_id)),
            ...
            // DELETE /stores/by_saga_id/:saga_id
            (&Delete, Some(Route::StoreBySagaId(saga_id))) => serialize_future(service.deactivate_store_by_saga_id(saga_id)),
            ...
            // GET /base_products/:id/validate_update
            (&Get, Some(Route::BaseProductValidateUpdate(base_product_id))) => {
                serialize_future(service.validate_update_base_product(base_product_id))
            }

            // GET /products/:id/validate_update
            (&Get, Some(Route::ProductValidateUpdate(product_id))) => serialize_future(service.validate_update_product(product_id)),
