...
use models::{NewModeratorStoreComments, ModeratorStoreComments};
    ...
    for ModeratorStoreRepoImpl<'a, T>
{
    ...
    fn find_by_store_id(&self, store_id_arg: i32) -> RepoResult<ModeratorStoreComments>{
        ...
        self.execute_query(moderator_store_comments.filter(store_id.eq(store_id_arg)).order_by(id.desc()).limit(1))
